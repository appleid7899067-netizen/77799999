...
function PuterChip() {
  ...
  if (!ready) {
    return (
      <span className="inline-flex h-8 items-center gap-1.5 rounded-full bg-elevated px-2.5 text-xs text-muted shadow-[var(--shadow-border)]">
            ...
            <DropdownMenuItem key={t.slug} asChild>
              <ToolLink tool={t} className="items-start">
                ...
                <span>
                  ...
                  <span className="block text-xs text-subtle">{t.blurb.slice(0, 42)}</span>
                </span>
              </ToolLink>
            </DropdownMenuItem>
          ))}
